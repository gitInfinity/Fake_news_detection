...
Choose a license for your project (e.g., MIT). Add a `LICENSE` file to the repo with the chosen license text.

---

## Contact & Contribution

If you'd like help reproducing or packaging the models, adding unit tests, or setting up CI to run training and deploy the Streamlit app, open an issue or create a PR with the suggested changes.

---

Enjoy working on the fake-news classifier. If you want, I can also:

- Add example test data and a small toy model for quick local demos, or
- Generate a minimal GitHub Actions workflow to lint, run unit tests, and optionally build artifacts.

Tell me which of those you'd like next and I can add it automatically.

---

## Docker (Optional): Build and Run in a Container

This repository includes Docker support so you can run the Streamlit app in a reproducible container. The Docker setup is intended for local development and simple deployment scenarios. It expects model files either in the project root or mounted into `/app/models` inside the container.

Files added for Docker:
- `Dockerfile` — builds a slim Python 3.11 image, installs system packages and Python deps, copies the app, and runs Streamlit.
- `entrypoint.sh` — downloads required NLTK data at container start and then runs the container command.
- `.dockerignore` — prevents large/unused files from being copied into the image.
- `docker-compose.yml` — convenience compose file to run the container and mount model files.
...
```powershell
# map port 8501 (Streamlit default) to host
docker run --rm -p 8501:8501 \
    -v "${PWD}:/app" \
    -v "${PWD}/models:/app/models" \
    fake-news-app:latest
```
...
- Model files are large and intentionally excluded from Git. Place `fake_news_classifier.h5` and `word2vec_model.model` in the `models/` folder or the project root and mount them into the container.
- The `entrypoint.sh` script downloads NLTK data (`punkt` and `stopwords`) on container start so the container is ready for inference.
- If you encounter TensorFlow runtime/DLL errors inside the container, try:
    - Using an official CPU TensorFlow wheel version (the Docker image uses `requirements.txt`).
    - Installing the correct CUDA/cuDNN toolchain only if you build a GPU image (not covered here).

Security and production notes
- For production, avoid mounting your entire project into the container; instead build an image that includes only the necessary artifacts (app + models + minimal runtime).
- Consider using a multi-stage Dockerfile to reduce image size, and run the app as a non-root user in production images.

If you want, I can:
- Add a multi-stage optimized Dockerfile (smaller final image), or
- Create a small demo model and sample data so the container works out-of-the-box for demoing.
